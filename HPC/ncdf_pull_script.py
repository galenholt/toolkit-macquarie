...
# def cut_marks(fromparent, toparent, mark, hews, clims, hs = ['historical', 'stochastic'], cutcdf = True, extrafiles = []):
#     for i in hs:
...
t = time.time()
cut_marks(basefrom, baseto, mark = 'MACQ_CC_EFR', hews = no_mgmt, clims = climpattern, extrafiles = extrafiles)
time.time()-t
...
t = time.time()
cut_marks(basefrom, baseto, mark = 'MACQ_CC_EFR_mkiv', hews = with_mgmt, clims = climpattern, extrafiles = extrafiles)
time.time()-t

# Mk5 (Only licvolfactor_1_0), so 6 climate * 76 stochastics = 456
t = time.time()
cut_marks(basefrom, baseto, mark = 'MACQ_CC_EFR_mkv', hews = no_mgmt, clims = climpattern, extrafiles = extrafiles)
time.time()-t

# Mk5a (Only licvolfactor_1_0), so 6 climate * 76 stochastics = 456
t = time.time()
cut_marks(basefrom, baseto, mark = 'MACQ_CC_EFR_mkva', hews = no_mgmt, clims = climpattern, extrafiles = extrafiles)
time.time()-t