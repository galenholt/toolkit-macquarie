...
# Historical
# We only want Ash's new runss
if (!file.exists('HPC/hist_dirs_ewrtarget.rds')) {
    hist_dirs <- list.dirs(file.path(project_dir, 'macq_cut/historical'), recursive = TRUE)
    hist_dirs <- hist_dirs[grepl('exp11', hist_dirs)]
    saveRDS(object = hist_dirs, file = 'HPC/hist_dirs_ewrtarget.rds')
} else {
    hist_dirs = readRDS('HPC/hist_dirs_ewrtarget.rds')
}
